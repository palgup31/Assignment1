...
m_1 <- quap(
  alist(
    ...
    b_smoking[SMOKING] ~ dnorm(0, 10)  # Prior for beta
  ),
  data = d
)
...
m_2 <- quap(
  alist(
    ...
    b_smoking[SMOKING] ~ dnorm(0, 0.5)  # Different prior for beta
  ),
  data = d
)
...
if (is.matrix(prior_1$b_smoking) & is.matrix(prior_2$b_smoking)) {
  
  # Apply inv_logit to a + b_gender for both models
  p1 <- sapply(1:2, function(k) inv_logit(prior_1$a + prior_1$b_smoking[,k]))
  p2 <- sapply(1:2, function(k) inv_logit(prior_2$a + prior_2$b_smoking[,k]))
  ...
  dens(b_1, col="blue", xlim=c(0, 1), ylim=c(0, 10), xlab="Beta (smoking)", main="Prior Comparison for Beta (smoking)")
